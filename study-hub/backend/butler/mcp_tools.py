...
_TOOL_SPECS = (
    ...
    ("butler_record_context", "记录已查到的项目记忆和领域知识。", {"case_id": {"type": "string"}, "project_index_hits": {"type": "array", "items": {"type": "string"}}, "owner_files": {"type": "array", "items": {"type": "string"}}}, ("case_id",)),
...
async def call_butler_tool(name: str, arguments: dict | None) -> list[TextContent]:
    ...
    try:
        handlers = {
            ...
            "butler_record_context": lambda: runtime.record_context(args["case_id"], project_index_hits=args.get("project_index_hits", []), owner_files=args.get("owner_files", [])),
