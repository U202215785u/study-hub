...
async def list_tools() -> list[Tool]:
    return [
        ...
        Tool(
            name="remember",
            description="记住一个关于用户的事实。当用户说'记住这个'或提到个人偏好、计划、习惯时使用。",
            inputSchema={
                "type": "object",
                "properties": {
                    "content": {"type": "string", "description": "事实内容，如'用户喜欢川菜'"},
                    "category": {"type": "string", "description": "分类: preferences / health / travel / work / people / learning / habits / skills / goals / other"},
                    "tags": {"type": "array", "items": {"type": "string"}, "description": "标签数组，如 ['food', 'spicy']"},
                    "importance": {"type": "integer", "description": "重要性 1-5，默认3"},
                },
                "required": ["content"],
            },
        ),
        Tool(
            name="recall",
            description="回忆与用户相关的记忆。每次对话开始时自动调用，注入相关上下文。",
            inputSchema={
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "搜索关键词，如'饮食偏好'"},
        ...
        Tool(
            name="forget",
            description="当用户纠正或否定之前的记忆时使用。支持自然语言描述（如'我不吃辣'）或直接指定记忆 ID。",
            inputSchema={
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "自然语言描述要遗忘的内容，如'我不吃辣'"},
                    "memory_id": {"type": "integer", "description": "可选：直接指定记忆 ID"},
                    "reason": {"type": "string", "description": "原因: outdated 或 wrong", "enum": ["outdated", "wrong"]},
                },
                "required": ["reason"],
            },
        ),
        Tool(
            ...
            description="从一段对话文本中自动提取事实并存入记忆系统。",
            inputSchema={
                "type": "object",
                "properties": {
        ...
        Tool(
            ...
            description="获取用户的完整记忆画像（五层记忆系统）。每次对话开始时自动调用，注入角色记忆、活跃项目、工作流偏好。",
            inputSchema={
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "当前对话主题或用户消息，用于检索最相关的记忆"},
                },
                "required": ["query"],
            },
        ),
        Tool(
            ...
            description="分析完整对话，分层提取记忆（角色/项目/工作流/会话）。对话结束时调用，自动保存到记忆系统。",
            inputSchema={
                "type": "object",
                "properties": {
                    "conversation": {"type": "string", "description": "完整对话文本"},
                    "detected_project": {"type": "string", "description": "检测到的项目名称（可选）"},
                },
...
async def call_tool(name: str, arguments: dict) -> list[TextContent]:
    try:
        ...
        # ===== 记忆系统 =====
            ...
            return await handle_extract_memories(arguments)
        # ===== 五层记忆系统 =====
        elif name == "get_context_profile":
            return await handle_get_context_profile(arguments)
        elif name == "summarize_conversation":
            return await handle_summarize_conversation(arguments)
        elif name == "list_projects":
            return await handle_list_projects(arguments)
        elif name == "list_workflows":
            return await handle_list_workflows(arguments)
        # ===== 社媒解析 =====
...
async def handle_remember(args: dict) -> list[TextContent]:
    body = {
        "content": args["content"],
    ...
    data = await api_post("/memory/remember", body)
    if "error" in data:
        return [TextContent(type="text", text=f"记住失败: {data['error']}")]
    tags = ", ".join(data.get("tags", []))
    return [TextContent(type="text", text=f"✅ 已记住 (ID {data['id']}): {data['content']}\n分类: {data.get('category', 'other')} | 标签: {tags}")]


async def handle_recall(args: dict) -> list[TextContent]:
    query = args["query"]
    top_k = args.get("top_k", 5)
    data = await api_get(f"/memory/recall?q={query}&top_k={top_k}")
    results = data.get("results", [])
    if not results:
    ...
    for r in results:
        ...
        lines.append(f"- (ID {r['id']}) {r['content']}{tag_str}")
    return [TextContent(type="text", text="\n".join(lines))]


async def handle_forget(args: dict) -> list[TextContent]:
    reason = args.get("reason", "wrong")
    ...
    data = await api_post("/memory/forget", body)
    if data.get("error"):
        return [TextContent(type="text", text=f"标记失败: {data['error']}")]

    if data.get("forgotten"):
        method = data.get("method", "direct")
        if method == "auto_match":
            return [TextContent(type="text", text=f"✅ 已自动匹配并标记记忆 #{data['id']} 为 {reason}。\n内容: {data.get('content', '')}\n置信度: {data.get('confidence', '')}")]
        return [TextContent(type="text", text=f"✅ 已标记记忆 #{data['id']} 为 {reason}。")]

    if data.get("needs_confirmation"):
        lines = ["找到多个候选记忆，请确认要标记哪一条："]
        for c in data.get("candidates", []):
            lines.append(f"- ID {c['id']}: {c['content']}")
        lines.append(f"\n请用 forget(memory_id=xxx, reason='{reason}') 指定要标记的记忆。")
        return [TextContent(type="text", text="\n".join(lines))]

    return [TextContent(type="text", text=f"未找到相关记忆。")]


async def handle_extract_memories(args: dict) -> list[TextContent]:
    body = {
        "text": args["text"],
        "source_tool": "claude",
    }
    data = await api_post("/memory/extract", body)
    added = data.get("added", 0)
    skipped = data.get("skipped", 0)
    items = data.get("items", [])
    text = f"提取完成：新增 {added} 条，跳过重复 {skipped} 条。\n"
    if items:
        text += "\n新增记忆：\n"
        for item in items:
            text += f"- {item['content']}\n"
    skipped_contents = data.get("skipped_contents", [])
    if skipped_contents:
        text += "\n跳过（已存在）：\n"
        for sc in skipped_contents:
            text += f"- {sc}\n"
    if data.get("error"):
        text += f"\n⚠️ 错误: {data['error']}"
    return [TextContent(type="text", text=text)]
...
async def handle_get_context_profile(args: dict) -> list[TextContent]:
    """获取用户记忆画像，用于对话开始时注入上下文"""
    query = args.get("query", "")
    data = await api_get(f"/memory/context_inject?q={encodeURIComponent(query)}&tool=claude")

    injection = data.get("injection", {})
    parts = []

    # 角色画像
    role = injection.get("role_profile", "")
    if role:
        parts.append(f"【用户画像】\n{role}")

    # 活跃项目
    projects = injection.get("active_projects", [])
    if projects:
        proj_lines = [f"- {p['name']}: {p.get('progress', '进行中')} (技术栈: {', '.join(p.get('tech_stack', []))})" for p in projects]
        parts.append(f"【活跃项目】\n" + "\n".join(proj_lines))

    # 工作流偏好
    workflows = injection.get("relevant_workflows", [])
    if workflows:
        wf_lines = []
        for w in workflows:
            prefs = w.get("preferences", {})
            pref_str = ", ".join(f"{k}={v}" for k, v in prefs.items()) if prefs else ""
            wf_lines.append(f"- {w['name']}: {pref_str}")
    ...
    if memories:
        ...
        for m in memories:
            ...
            label = {"role": "[角色]", "project": "[项目]", "workflow": "[工作流]", "session": "[会话]", "world": "[知识]"}.get(layer, "")
            mem_lines.append(f"- {label} {m.get('content', '')}")
        parts.append(f"【相关记忆】\n" + "\n".join(mem_lines))

    if not parts:
        return [TextContent(type="text", text="暂无记忆画像。随着对话进行，系统会自动学习和记录。")]

    full_text = "\n\n".join(parts)
    return [TextContent(type="text", text=full_text)]


async def handle_summarize_conversation(args: dict) -> list[TextContent]:
    """对话结束时调用，分层提取并保存记忆"""
    body = {
        "conversation": args["conversation"],
        "source_tool": "claude",
        "detected_project": args.get("detected_project", ""),
    }
    data = await api_post("/memory/summarize_and_extract", body)

    if data.get("error"):
        return [TextContent(type="text", text=f"记忆提取失败: {data['error']}")]

    added = data.get("added", 0)
    layers = data.get("added_by_layer", {})
    summary = data.get("summary", "")

    text = f"🧠 对话已分析，提取 {added} 条记忆\n"
    if summary:
        text += f"\n总结: {summary}\n"
    text += f"\n分层统计:\n"
    text += f"- 角色记忆: {layers.get('role', 0)} 条\n"
    text += f"- 项目记忆: {layers.get('project', 0)} 条\n"
    text += f"- 工作流记忆: {layers.get('workflow', 0)} 条\n"
    text += f"- 会话记忆: {layers.get('session', 0)} 条\n"

    projects = data.get("projects_detected", [])
    if projects:
        text += f"\n检测到的项目: {', '.join(projects)}"
...
async def handle_list_projects(args: dict) -> list[TextContent]:
    ...
    if not data:
        return [TextContent(type="text", text="暂无项目。")]
    ...
    for p in data:
        ...
        lines.append(f"{status_icon} {p['name']}{tech_str} — {p.get('progress_note', '进行中')}")
    return [TextContent(type="text", text="\n".join(lines))]


async def handle_list_workflows(args: dict) -> list[TextContent]:
    """列出用户工作流"""
    data = await api_get("/workflows")
    if not data:
        return [TextContent(type="text", text="暂无工作流偏好。")]
    lines = [f"共 {len(data)} 个工作流："]
    for w in data:
        keywords = ", ".join(w.get("trigger_keywords", []))
        kw_str = f" (触发: {keywords})" if keywords else ""
        prefs = w.get("preferences", {})
        pref_str = ", ".join(f"{k}={v}" for k, v in prefs.items()) if prefs else ""
        lines.append(f"⚙️ {w['name']}{kw_str} — {pref_str}")
    return [TextContent(type="text", text="\n".join(lines))]
