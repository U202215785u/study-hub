...
- 每个新摄入的文档，都应**更新和丰富已有页面**，而不仅仅是追加新页面
- 知识库的价值在于**交叉引用**——页面之间通过 [[wikilink]] 形成知识网络
  ...
  "new_pages": [
    {
      ...
      "content": "完整的 Markdown 内容，使用 [[slug]] 引用其他 Wiki 页面",
      "summary": "1-2句概述",
      "tags": ["标签1", "标签2"],
      "category": "分类名"
    }
...
## 写作规则
1. **使用 [[slug]] 语法**做内部链接，在 content 中以 `[[slug]]` 或 `[[slug|显示文本]]` 引用其他页面
2. **新页面**：提取文档中独立的知识主题，每个主题一个页面。页面内容结构化：概述 → 核心概念 → 详细说明 → 实践/示例 → 相关资源
3. **更新已有页面**：如果新内容与已有页面相关，追加到该页面底部并标注 `> 更新于 {date}，来源：{source}`
4. **矛盾检测**：如果新内容与已有知识冲突，标记矛盾
5. **避免重复**：如果新内容与已有页面高度重复，只做小增量更新
6. **中文为主**，专业术语保留英文
7. 每个页面的 content 应当完整、自包含，读者不需要看原始文档就能理解
8. content 中使用 Markdown 格式：标题用 ##/###，代码用代码块，重要概念用 **加粗**
"""
