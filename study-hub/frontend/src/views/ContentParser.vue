<template>
  <div class="min-h-screen bg-[#0b1012] text-text flex">
    ...
    <main class="flex-1 min-w-0 p-5 md:p-10 max-w-[1440px]">
      ...
      <div v-if="document" data-testid="document-backdrop" class="fixed inset-0 z-50 bg-black/70 p-4 md:p-12" @click.self="document = null"><section class="h-full max-w-5xl mx-auto bg-[#11191b] border border-border rounded-[6px] p-6 overflow-y-auto"><button type="button" @click="document = null" class="float-right text-text-secondary">关闭</button><h2 class="text-xl font-bold mb-6">{{ document.title }}</h2><section v-if="documentFailure" class="mb-6 border border-danger/60 bg-danger/10 p-4 rounded-[6px] text-sm" role="alert"><div class="font-semibold text-danger">解析未完成</div><code class="block mt-2 font-mono text-danger">{{ documentFailure.code }}</code><p class="mt-2 text-text-secondary">{{ documentFailure.reason }}</p></section><MarkdownRenderer :content="document.content" /></section></div>
    </main>
  </div>
</template>
<script setup>
