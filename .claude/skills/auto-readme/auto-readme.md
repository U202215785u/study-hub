...
trigger: 用户说 "自动文档" "auto readme" "自动更新readme" "代码文档自动同步" "每次改代码自动更新文档" "README自动生成"
---
...
- 测试文件位置：`.claude/scripts/test_generate_readme.py`
- 测试内容：
...
在 `.claude/scripts/generate_readme.py` 生成一个完整的静态分析脚本。
...
在 `.claude/settings.json` 中配置 PostToolUse hook。
...
3. 运行 `python .claude/scripts/generate_readme.py --write` 填充自动区
4. 运行 `--check` 验证一切正常
...
手动触发：
  python .claude/scripts/generate_readme.py --write   # 强制刷新
  python .claude/scripts/generate_readme.py --check   # 仅检查
...
5. **合并已有 settings.json**：如果 `.claude/settings.json` 已有其他配置（如 permissions），只追加 hooks 部分
...
Claude：
  ...
  （第二步：生成脚本）
  正在生成 .claude/scripts/generate_readme.py ...
  [包含 Python 解析器 + FastAPI 路由提取 + SQLite schema 提取]

  （第三步：配置 Hook）
  正在配置 .claude/settings.json ...
