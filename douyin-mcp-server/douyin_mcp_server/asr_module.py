...
class QwenASR:
    ...
    def recognize_audio(
        ...
        try:
            ...
            if response.status_code != 200:
                raise DouyinError(
                    DY_4001_ASR_CALL_FAILED,
                    detail=f"HTTP {response.status_code}: {response.message}",
                )
            ...
            err = e if isinstance(e, DouyinError) else DouyinError(DY_4002_ASR_RECOGNITION_FAILED, detail=str(e))
            log_error(logger, err.error_code, err.message, exc=e, context=f"model={self.model}")
            return {
                "success": False,
    ...
    def recognize_file(
        ...
        if not file_path.exists():
            err = DouyinError(DY_3003_FILE_NOT_FOUND, detail=str(file_path))
