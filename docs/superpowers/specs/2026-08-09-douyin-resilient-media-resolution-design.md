...
验证和写入视频临时文件的组件，并下沉到 `douyin-mcp-server` 作为可复用组件。
Study-Hub 后端和 MCP 工具共同消费这一组件，避免任一入口绕过候选回退与响应校验。

## 5. 媒体候选契约

`DouyinProcessor.parse_share_url()`（契约版本 `1.4.0`）对视频返回现有字段，并新增
...
- MCP 工具 `get_douyin_download_link` 保留兼容字段 `download_url`，但其值必须来自
  共享下载器选中的已验证候选，并附带候选诊断；不得直接返回未经验证的 `url`。
- MCP 工具 `extract_douyin_text` 使用共享下载器选择候选；若 ASR 的 URL 拉取路径对
  当前候选失败，按候选顺序继续尝试，耗尽后才返回下载层错误。
...
- 白名单覆盖 `douyin.com`、`douyinvod.com`、`douyinpic.com`、`douyinstatic.com`、
  `douyincdn.com`、`byteimg.com`、`bytecdn.cn`、`bytedance.com`、`snssdk.com`、
  `zjcdn.com` 的合法子域；示例中的 `sf6-cdn-tos.douyinstatic.com` 因此可被接受。
- 白名单基于真实解析结果滚动补充，但不在运行时自动学习；新增根域后缀必须提供观测到
  的解析证据，完成 DNS/重定向/SSRF 安全评审、补充测试并通过代码评审后才能入库。
...
6. 流式读取首块，目标为 4 KB；若提前 EOF，凡累计小于 4 KB（包括 0 字节和
   1 KB～4 KB）一律判定为 `empty_media_response`。只有取得完整 4 KB 后才进入下一步。
7. 对视频候选在完整首块的前 4 KB 中校验 ISO BMFF/MP4 `ftyp` 标记；不匹配判定为
...
| 错误码 | 条件 | 用户提示 |
|---|---|---|
| `DY-2010` | 所有候选均返回 404/410 | 视频媒体地址已失效，请重新提交链接后重试。 |
| `DY-2011` | 响应在取得完整 4 KB 前 EOF | 抖音播放地址返回不完整数据，已尝试其它地址但未成功。 |
| `DY-2012` | HTML、JSON、空 MIME 或无效文件头 | 抖音返回的不是可处理的视频数据。 |
| `DY-2013` | 403、429、验证码/风控 | 抖音暂时限制访问，请稍后重试。 |
| `DY-2014` | 传输错误或 5xx 重试耗尽 | 抖音媒体暂时无法访问，请稍后重试。 |
| `DY-2005` | 页面和 F2 都没有可处理媒体 | 未发现可处理的视频或图文媒体。 |

`DY-2005` 只允许由解析层在候选发现阶段抛出；下载层失败一律使用 `DY-2010`～
`DY-2014`，不得复用 `DY-2005`。新任务、UI 和日志只产生 `DY-*`；历史记录保留原有
`PARSER-ASR-*` 文本，不按新语义重新分类。任务详情保存来源、HTTP 状态、内容类型、
字节数和候选编号，但 UI 仅展示安全的摘要。

## 10. 兼容与迁移

- 新契约将 `douyin-mcp-server` 版本提升到 `1.4.0`；启动兼容性检查同步要求
  `>= 1.4.0`，并报告实际版本和源码路径。
- 解析器先同时返回 `media_candidates` 与旧 `url`/`video_url`，后端切换到候选
  下载器后再逐步减少旧字段依赖。
- 迁移期间新后端兼容旧解析器：若响应没有 `media_candidates`，仅将旧 `video_url`
  包装为 `legacy.video_url` 单一候选，并明确记录“候选来源诊断不可用”。
...
- 旧解析器（< 1.4.0）+ 新后端的降级测试：任务仍可执行或明确失败，且不声称拥有
  候选来源诊断；新解析器 + 旧后端的兼容行为也需记录为发布阻断项。
...
- `get_douyin_download_link` 与 Study-Hub 后端对同一候选集合必须选中同一地址，且 MCP
  返回的 `download_url` 已通过首块校验。
- `extract_douyin_text` 的首个候选被 ASR URL 拉取拒绝时，断言会改用下一已验证候选；
  所有候选失败时只返回对应的 `DY-2010`～`DY-2014` 下载层错误。
...
6. 运行时解析器版本为 `>= 1.4.0`；旧解析器降级时有明确的兼容诊断，而不是静默按新
   候选契约处理。
...
- 删除字符串替换，页面适配器输出候选列表。
- 在 `douyin-mcp-server` 落地共享候选下载器，后端与两个 MCP 工具均通过它按候选下载、
  回退并实现准确错误分类。
- 修复本次 0 字节误报。
